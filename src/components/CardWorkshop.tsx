...
export function CardWorkshop({ workshop }: { workshop: Workshop }) {
  return (
    <motion.article whileHover={{ y: -4 }} className="glass-card group rounded-2xl p-5 transition duration-300 hover:border-orange-300/45 hover:shadow-xl hover:shadow-black/30">
      ...
      <p className="mt-2 text-sm text-zinc-400">{new Date(workshop.data_evento).toLocaleDateString('pt-BR')} • {workshop.local}</p>
